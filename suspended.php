...
$sql = "SELECT username, email, suspension_end_date FROM users WHERE id='$user_id'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $user = $result->fetch_assoc();
    $suspension_end_date = new DateTime($user['suspension_end_date']);
    $today = new DateTime('today');

    if ($suspension_end_date < $today) {
        // If suspension end date is in the past, set days remaining to 0
        $days_remaining = 0;
    } else {
        $interval = $suspension_end_date->diff($today);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $admin_email = 'tessnarval11@gmail.com'; // Replace this with the actual admin email
    ...
    <?php if(isset($_SESSION["success_message"])): ?>
        ...
        <p>Your account has been suspended for <?php echo $days_remaining; ?> days.</p>
